            ...
            <div className='relative'>
                ...
                {isPaymentDropdownOpen && (
                    <ul className='absolute w-full z-40 border border-[#e8e8e8] border-t-0 bg-white rounded-b-md max-h-[300px]'>
                        <li
                            className={`py-[6px] px-3 max-h-9 text-[#35495e] text-sm transition-all duration-300 cursor-pointer flex items-center justify-between ${selectedMods.includes('delivery') ? 'bg-gray-100 font-medium' : 'hover:bg-black hover:text-white'}`}
                            onClick={() => selectMod('delivery')}
                        >
                            delivery

                            {selectedMods.includes('delivery') && <span className='font-semibold opacity-70'>selected</span>}
                        </li>
                    </ul>
                )}
            </div>
