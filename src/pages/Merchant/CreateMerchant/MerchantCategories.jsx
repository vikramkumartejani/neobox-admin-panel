...
const MerchantCategories = () => {
    ...
    return (
        <div className='pb-4'>
            <label htmlFor="merhcant_category" className="pb-2 block font-medium text-sm">
                Merchant Category
            </label>
            ...
            <div className='relative'>
                ...
                {isPaymentDropdownOpen && (
                    <ul className='absolute w-full z-40 border border-[#e8e8e8] border-t-0 bg-white rounded-b-md max-h-[300px]'>
                        <li
                            className={`py-[6px] px-3 max-h-9 text-[#35495e] text-sm transition-all duration-300 cursor-pointer flex items-center justify-between ${selectedMods.includes('burgar') ? 'bg-gray-100 font-medium' : 'hover:bg-black hover:text-white'}`}
                            onClick={() => selectMod('burgar')}
                        >
                            Burgar

                            {selectedMods.includes('burgar') && <span className='font-semibold opacity-70'>selected</span>}
                        </li>
                        <li
                            className={`py-[6px] px-3 max-h-9 text-[#35495e] text-sm transition-all duration-300 cursor-pointer flex items-center justify-between ${selectedMods.includes('clothhing') ? 'bg-gray-100 font-medium' : 'hover:bg-black hover:text-white'}`}
                            onClick={() => selectMod('clothhing')}
                        >
                            Clothhing

                            {selectedMods.includes('clothhing') && <span className='font-semibold opacity-70'>selected</span>}
                        </li>
                    </ul>
                )}
            </div>
